...
<div class="max-w-3xl mx-auto px-6 py-12 mt-8">
    <div class="mb-8">
        ...
        <p class="text-slate-500 mt-1">Ubah detail acara kamu.</p>
    </div>

    <div class="bg-white p-8 rounded-2xl border border-slate-200 shadow">
        <form action="{{ route('organizer.events.update', $event->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="datetime-local" name="date" value="{{ old('date', \Carbon\Carbon::parse($event->date)->format('Y-m-d\TH:i')) }}" class="w-full px-5 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none transition font-medium" required>
            ...
            <div>
                ...
                @if($event->poster_path && \Illuminate\Support\Facades\Storage::disk('public')->exists($event->poster_path))
                    <img src="{{ asset('storage/' . $event->poster_path) }}" class="w-24 h-32 object-cover rounded-xl mb-3 shadow-sm">
                @endif
                ...
                <p class="text-xs text-slate-400 mt-1">Kosongkan kalau gak mau ganti poster.</p>
